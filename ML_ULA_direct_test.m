...
RISconfigs = Dh_angles*arrayresponse(RIS_directions,M) - ones (M,1);
B = RISconfigs';
%%
...
for itr = 1:M-1
    ...
    for i = 1:length(varphi_range)
        ...
        utility_den(i) = norm(B*Dh*arrayresponse(varphi_range(i),M))^2 - (itr+1)^-1 * abs(ones(1,itr+1)*B*Dh*arrayresponse(varphi_range(i),M))^2;
    end
